...
export function highlightRequiredFields() {
...
export function setErrorMessage(element) {
    let parent = element.parentNode;
    setTimeout(() => {
        parent.classList.add('md-invalid');
    }, 1);
}
...
export function performValidation(id, value, validations) {
    if (validations && validations.length > 0) {
        let isValid = handleValidation(validations, value);
        if (isValid) {
            removeErrorMessage(document.getElementById(id));
        }
        else {
            setErrorMessage(document.getElementById(id));
        }
        return isValid;
    }
}

export function handleNumberValidation(id, value, min, max, validations) {
    let isValid;
    if (value < min || value > max) {
        ...
        isValid = performValidation(id, value, validations);
        isValid;
    }
    return isValid;
}

export function handleValidation(validations, value) {
    let isValid = true;
    for (let i = 0; i < validations.length; i++) {
        let validation = validations[i];
        if (!getValidationCase(validation, value)) {
            isValid = false;
            break;
        }
    }
...
export function getValidationCase(validation, value) {
    let re;
    switch (validation) {
        case "required":
